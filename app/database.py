...
class User(Base):
    ...
    # Relacionamentos
    cases_as_patient = relationship(
        'Case',
        back_populates='patient',
        foreign_keys='Case.patient_id'
    )
    cases_as_doctor = relationship(
        'Case',
        back_populates='doctor',
        foreign_keys='Case.doctor_id'
    )
    # REMOVIDO: documents = relationship('Document', back_populates='case')
...
class Case(Base):
    ...
    patient = relationship(
        'User',
        back_populates='cases_as_patient',
        foreign_keys=[patient_id]
    )
    doctor = relationship(
        'User',
        back_populates='cases_as_doctor',
        foreign_keys=[doctor_id]
    )
    document = relationship(
        'Document',
        back_populates='case',
        uselist=False  # Um caso tem um documento
    )
